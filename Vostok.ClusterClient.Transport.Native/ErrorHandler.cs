...
namespace Vostok.Clusterclient.Transport.Native
{
    internal class ErrorHandler
    {
        ...
        private void LogWin32Error(Request request, Win32Exception error)
            => log.Error(error, "WinAPI error with code {ErrorCode} occured while sending request to {Target}.", error.NativeErrorCode, request.Url.Authority);

        private void LogCurlError(Request request, Exception error, CurlCode code)
            => log.Error(error, "CURL error with code {ErrorCode} occured while sending request to {Target}.", code, request.Url.Authority);
